...
# ==============================================================================
# 📦 BLOCK 1: CẤU HÌNH HỆ THỐNG LÕI
# ==============================================================================
class Config:
    VERSION = "V36.1.2 PRO ALGO (KIẾN TRÚC BLOCK - TÍCH HỢP GIAO DIỆN NHẬP TAY)"
    ...
    MENU_OPTIONS = [
        ...
        "📈 5. ĐẠI KẾ TOÁN QUÉT CHU KỲ TỔNG HỢP",
        "🎰 6. DỮ LIỆU THÔ"
    ]

# ==============================================================================
# 🛠️ BLOCK 2: CÔNG CỤ XỬ LÝ LÕI & ĐỒNG BỘ THỜI GIAN
# ==============================================================================
class Utils:
    @staticmethod
    def get_vn_time():
...
# ==============================================================================
# 🕸️ BLOCK 3: TRÌNH THU THẬP DỮ LIỆU ĐA TẦNG (CRAWLER XUYÊN THẤU)
# ==============================================================================
class Crawler:
    @staticmethod
...
class DatabaseManager:
    ...
    def load_db():
        ...
        try:
            ...
            if dup_count > 0: msg += f" (Cảnh báo: Đã ghi đè {dup_count} dòng trùng)."
            return db, msg
    ...
    def auto_heal_history():
        ...
        return "✅ AUTO-HEAL: Dữ liệu an toàn, không có lỗ hổng 15 ngày qua."
...
# ==============================================================================
# 🧠 BLOCK 5: QUANT ENGINE (GIỮ NGUYÊN LÕI GỐC V36.1.1 ĐỂ BẢO ĐẢM MINH BẠCH)
# ==============================================================================
class QuantEngine:
    @staticmethod
...
# ==============================================================================
# 📊 BLOCK 6: HỆ THỐNG AUDITOR KẾ TOÁN
# ==============================================================================
class Auditor:
    @staticmethod
    ...
    def phan_he_2_predict(pts_per_code_base, mode):
        try:
            ...
            if dan is None:
                lines.extend([f"🛑 CẢNH BÁO RỦI RO: Dữ liệu tham chiếu {msg}.", "👉 HỆ THỐNG TỰ ĐỘNG TẠM NGỪNG CẤP TÍN HIỆU ĐỂ BẢO TOÀN VỐN."])
                return "\n".join(lines)
    ...
    def phan_he_3_risk(base_pts, sim_size):
        try:
            ...
            base_pts, so_luong_lo = Utils.safe_int(base_pts), Utils.safe_int(sim_size)
            von_ngay = so_luong_lo * base_pts * Config.COST_PER_POINT
            lines = [
    ...
    def phan_he_4_single(ngay_raw, pts_per_code_base):
        try:
            ...
            if sl_r == 0: report += f" 👉 KẾT QUẢ: 100% Danh mục duy trì động lượng tốt (Không có mã khuyết nhịp)\n\n"
            else:
            ...
            if sl_t == 0: report += f" 👉 KẾT QUẢ: KHÔNG CÓ MÃ ĐẠT CHUẨN (Toàn bộ danh mục mất động lượng)\n\n"
            else:
    ...
    def phan_he_5_range(tu_ngay_raw, den_ngay_raw, pts_per_code_base, mode):
        try:
            ...
            lines = [
                "📑 [PHÂN HỆ 5] BÁO CÁO: ĐẠI KẾ TOÁN QUÉT CHU KỲ (ĐÃ GỘP BÁO CÁO THÁNG)",
                "===================================================================================================================",
            ...
            if not daily_records: return "\n".join(lines) + "🛑 KHÔNG CÓ PHIÊN GIAO DỊCH NÀO ĐẠT ĐIỀU KIỆN XUẤT LỆNH THỰC TẾ."
            
            df_rec = pd.DataFrame(daily_records)
            
            # 1. BẢNG TỔNG HỢP THEO NĂM
            lines.extend([
            ...
            for m_str, g_m in df_rec.groupby("month_str", sort=False):
                ...
                lines.append(f"Tháng {m_str:<5} | {len(g_m):<7} | {g_m['win'].sum()}W/{g_m['loss'].sum()}L | {m_chi:<14,.0f} | {m_thu:<14,.0f} | {m_lai:>+16,.0f} | {m_roi:>+7.2f}%")
            ...
            lines.extend([
                "===================================================================================================================",
                f"📝 ĐẠI KẾ TOÁN TỔNG CỘNG ({len(df_rec)} PHIÊN CÓ XUẤT LỆNH | Win: {df_rec['win'].sum()} - Loss: {df_rec['loss'].sum()}):",
    ...
    def phan_he_6_raw(ngay_raw):
        try:
            ...
            lines = [
                "📑 [PHÂN HỆ 6] BÁO CÁO: TRUY XUẤT RAW DB (DỮ LIỆU THÔ THỨ TỰ LỒNG CẦU)",
                "=======================================================",
...
def create_ui():
    ...
    with gr.Blocks(title="XSMB QUANT V36.1.2 PRO", theme=gr.themes.Soft()) as demo:
        gr.Markdown("# 🚀 XSMB QUANT V36.1.2 PRO — BẢN TÍCH HỢP BẢO MẬT & NHẬP LIỆU THỦ CÔNG")
        gr.Markdown("*(Hệ thống đã Đóng Block vững chắc. Hỗ trợ song song Crawler tự động và Nhập Tay khi Web bảo trì.)*")
        ...
        with gr.Column(visible=False) as col_2:
            with gr.Row():
                ...
                mode_2 = gr.Radio(choices=Config.MODES, value=Config.MODES[1], label="Chiến lược Áp dụng")
        ...
        with gr.Column(visible=False) as col_5:
            with gr.Row():
                ...
                mode_5 = gr.Radio(choices=Config.MODES, value=Config.MODES[1], label="Chiến lược Áp dụng")
            btn_5 = gr.Button("📈 KIỂM TOÁN ĐẠI KẾ TOÁN QUÉT CHU KỲ", variant="primary")
            out_5 = gr.Textbox(label="Báo cáo Tổng Dòng Tiền & Kế Toán Tháng", lines=22)
