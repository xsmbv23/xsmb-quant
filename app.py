...
class StockQuantEngine:
    ...
    def fetch_stock_data(ticker, days=180):
        ...
        # 1. ENTRADE OPEN API (Ưu tiên)
        try:
            end_date = get_vn_time()
            start_date = end_date - timedelta(days=days + 60)
            url = f"https://services.entrade.com.vn/chart-api/v2/ohlcs/stock?resolution=D&symbol={ticker_clean}&from={int(start_date.timestamp())}&to={int(end_date.timestamp())}"
        ...
        try:
            url = f"https://query1.finance.yahoo.com/v8/finance/chart/{ticker_clean}.VN?range=1y&interval=1d"
            res = requests.get(url, headers={'User-Agent': 'Mozilla/5.0'}, timeout=6)
            if res.status_code == 200:
                data = res.json()
    ...
    def update_master_db(tickers_str, days=180):
        ...
        return summary, ", ".join(old_tickers) if old_tickers else "Chưa có", ", ".join(new_tickers) if new_tickers else "Không có mã mới", master_file
    ...
    def run_radar_analytics():
        ...
        try:
            ...
            if not results:
                return pd.DataFrame(), "🛑 Không đủ dữ liệu tính toán (Cần >= 20 phiên/mã)."

            res_df = pd.DataFrame(results).sort_values(by='Score', ascending=False).reset_index(drop=True)

            if res_df.empty:
                return pd.DataFrame(), "🛑 Không tìm thấy mã nào đủ điều kiện."
