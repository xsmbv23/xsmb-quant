...
# ==============================================================================
# 🧬 HẠ TẦNG QUANT V27.0 - STOP-LOSS SHIELD MASTER
# ==============================================================================
VERSION = "V27.0 STOP-LOSS SHIELD MASTER"
...
# ==============================================================================
# 🎯 LÕI V27.0: BỘ LỌC MULTI-BRIDGE & 2 LỚP PHANH STOP-LOSS
# ==============================================================================
def tinh_cap_lo_v27_shield(target_dt, db, user_base_pts=10, luy_ke_hien_tai=0.0, streak_loss=0, stop_loss_limit=-2000000):
    
    # 🛡️ LỚP GIÁP 1: CẦU DAO CẮT LỖ CỨNG (HARD STOP-LOSS)
    if luy_ke_hien_tai <= stop_loss_limit:
        return [], False, f"☠️ DEAD ZONE (KHÓA TÀI KHOẢN): Lỗ {luy_ke_hien_tai:,.0f}đ đã chạm ngưỡng Cắt lỗ cứng ({stop_loss_limit:,.0f}đ). HỆ THỐNG TỪ CHỐI GIAO DỊCH!", 0.0, {}, 0, "HARD_STOP_LOSS"
    ...
    # 🎚️ VAN ĐIỀU TIẾT DÒNG VỐN & PHANH KỶ LUẬT DU KÍCH (Ngưỡng 7.50)
    if a4_score < 7.5:
        gate_status = "SKIP"
        is_trade = False
        final_pairs = []
        actual_pts = 0
        reason = f"🛑 LOW SIGNAL (PHANH DU KÍCH): Cầu xấu nhiễu (Score = {a4_score:.2f} < 7.50) -> Đứng ngoài bảo toàn vốn"
    ...
    elif 7.5 <= a4_score < 9.5:
        gate_status = "BALANCED_FLOW"
        is_trade = True
        final_pairs = top_4_pairs[:2]
        actual_pts = max(1, int(user_base_pts * 0.5))
        reason = f"🌗 BALANCED FLOW (CÂN BẰNG): Tín hiệu tốt (Score = {a4_score:.2f}) -> Đánh 2 Cặp, cược 50% tiền"
    else:
        gate_status = "FULL_POWER"
        is_trade = True
        final_pairs = top_4_pairs
        actual_pts = int(user_base_pts)
        reason = f"🔥 FULL POWER (TẤN CÔNG): Tín hiệu cực nét (Score = {a4_score:.2f} >= 9.50) -> Đánh 4 Cặp, cược 100% tiền"
...
def truy_xuat_trang_thai_real(db, base_pts, cost_pt, target_dt, stop_loss_limit):
    ...
    for d_obj in all_dates:
        ...
        pairs, is_trade, _, _, _, actual_pts, gate_status = tinh_cap_lo_v27_shield(
...
# ==============================================================================
# 🖥️ GIAO DIỆN GRADIO V27.0 - TÍCH HỢP BIẾN STOP-LOSS VÀO UI
# ==============================================================================
def web_phan_he_1_sync():
    ...
    res = f"📡 KẾT NỐI HỆ THỐNG QUANT V27.0 STOP-LOSS SHIELD MASTER:\n"
    res += f"---------------------------------------------------------------------------------\n"
...
def web_phan_he_2_predict(cost_per_point, pts_per_code_base, sl_limit):
    try:
        ...
        pairs, is_trade, reason, sc, p_details, actual_pts, gate_status = tinh_cap_lo_v27_shield(
        ...
        res = f"🎯 BÁO CÁO DỰ ĐOÁN V27.0 (SHIELD) CHO KỲ: {next_predict_dt.strftime('%d/%m/%Y')}\n"
        res += f"📌 (Ngưỡng Cắt Lỗ Cứng kích hoạt tại: {stop_loss:,.0f} VND)\n"
        res += f"---------------------------------------------------------------------------------\n"
...
def web_phan_he_6_range_performance(tu_ngay_raw, den_ngay_raw, cost_per_point, pts_per_code_base, sl_limit):
    try:
        ...
        report = f"📈 BÁO CÁO CHU KỲ (CÓ ÁP DỤNG STOP-LOSS TẠI {stop_loss:,.0f}đ):\n"
        report += f"=======================================================================================================================================\n"
        ...
        report += f"=======================================================================================================================================\n"
        report += f"📊 THỐNG KÊ CHU KỲ (CÓ PHANH DU KÍCH & STOP-LOSS):\n"
...
# ==============================================================================
# 🎨 GIAO DIỆN GRADIO V27.0
# ==============================================================================
db_init, _ = doc_database_tu_excel()
latest_dt_init, next_predict_dt_init = lay_ngay_chot_tu_excel(db_init)

with gr.Blocks(title="XSMB QUANT V27.0 SHIELD") as demo:
    gr.Markdown("# 🛡️ XSMB QUANT V27.0 — STOP-LOSS SHIELD MASTER")
    ...
    with gr.Tab("🎯 [2] Dự Đoán Có Phanh"):
        ...
        with gr.Row():
            ...
            sl_2 = gr.Number(label="Ngưỡng Cắt Lỗ Cứng (VND, Vd: -2000000)", value=-2000000)
        btn_2 = gr.Button("🔍 TRÍCH XUẤT LỆNH VÀ KIỂM TRA STOP-LOSS", variant="primary")
        out_2 = gr.Textbox(label="Hồ sơ Lệnh V27.0", lines=14)
    ...
    with gr.Tab("🔍 [4] Backtest (Kèm Stop-loss)"):
        ...
        btn_4 = gr.Button("📡 BACKTEST V27.0", variant="primary")
    ...
    with gr.Tab("📈 [6] Quét Chu Kỳ V27.0"):
        with gr.Row():
        ...
        btn_6 = gr.Button("📈 QUÉT CHU KỲ (CÓ PHANH & STOP-LOSS)", variant="primary")
