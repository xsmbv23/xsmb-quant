...
VERSION = "V36.2 PRO ALGO (ĐÃ NÂNG CẤP BỘ LỌC ĐỘNG LƯỢNG KHÁNG ÂM EV)"
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if len(loto_list) >= 27:
                ...
                db[ngay_str] = {'date_obj': dt_obj, 'date_str': ngay_str, 'prizes_int': loto_list[:27]}
        msg = f'🟢 ĐỒNG BỘ THÀNH CÔNG {len(db)} PHIÊN GIAO DỊCH.'
        if dup_count > 0: msg += f' (Cảnh báo: Phát hiện {dup_count} dòng trùng ngày đã ghi đè).'
        return db, msg
...
def get_signal_v36(target_dt, db, mode):
    ...
    if mode == '🚀 Giao Dịch T-7 ĐỘNG LƯỢNG TỐI ƯU (Cải Tiến Quant V36.2)':
        recent_3d = set()
        for d in range(1, 4):
    ...
    elif mode in ['Chỉ Giao Dịch TINH HOA (Lọc Số Khuyết)', 'Chỉ Giao Dịch SỐ KHUYẾT (Không Rơi/Đảo)']:
        ...
        if mode == 'Chỉ Giao Dịch TINH HOA (Lọc Số Khuyết)': return sorted(list(tinh_hoa)), 'OK'
...
def web_phan_he_2_predict(pts_per_code_base, mode):
    try:
        ...
        if dan is None:
            ...
            lines.append('HỆ THỐNG TỰ ĐỘNG TẠM NGỪNG CẤP TÍN HIỆU ĐỂ BẢO TOÀN VỐN.')
            return '\n'.join(lines)
        so_luong_lo = len(dan)
        ...
        if so_luong_lo > 0:
            ...
            diem_hoa_von_nhay = math.ceil(von_ngay / (base_pts * WIN_PER_NHAY)) if base_pts > 0 else 0
            lines.append(f'💡 MỤC TIÊU HÒA VỐN   : Cần tối thiểu {diem_hoa_von_nhay} lượt trúng.')
        else:
            lines.append('📋 DANH MỤC MÃ SỐ ĐẠT CHUẨN:')
            lines.append(' 👉 🚫 [KHÔNG CÓ TÍN HIỆU KHẢ THI]')
            lines.append('-------------------------------------------------------')
            lines.append('💰 TỔNG VỐN YÊU CẦU: 0 VND')
            lines.append('💡 HỆ THỐNG KHUYẾN NGHỊ ĐỨNG NGOÀI THỊ TRƯỜNG TRONG PHIÊN NÀY.')
        return '\n'.join(lines)
...
def web_phan_he_4_single_day_backtest(ngay_raw, pts_per_code_base):
    try:
        ...
        if ngay_str_t7 not in db:
            ...
            lines.append(f'🔭 LỖI CHU KỲ: Thiếu dữ liệu mốc T-7 ({ngay_str_t7}). Không thể phân tích!')
            return '\n'.join(lines)
        ...
        lines.append(f'📡 KẾT QUẢ GIAO DỊCH PHIÊN: {ngay_str} (Phân bổ: {base_pts}đ/mã)')
        lines.append('')
        lines.append('🛑 [KỊCH BẢN 1] - GIAO DỊCH TOÀN BỘ T-7 (Tinh hoa + Khuyết nhịp)')
        lines.append(f' • Danh mục {sl_f} mã: ' + ' '.join([f'{x:02d}' for x in list_full]))
        lines.append(f' • Đạt {nhay_f} lượt.  Vốn đầu tư: {chi_f:,.0f}đ  | Doanh thu: {thu_f:,.0f}đ')
        lines.append(f' 👉 LỢI NHUẬN RÒNG: {lai_f_str} VNĐ ({st_f})')
        lines.append('')
        if ngay_str_t1 not in db:
            lines.append(f'⚠️ LƯU Ý: Thiếu dữ liệu mốc T-1 ({ngay_str_t1}).')
            lines.append('Không thể phân tách rủi ro cho Kịch bản 2 và 3.')
            return '\n'.join(lines)
        ...
        lines.append('📉 [KỊCH BẢN 2] - BÓC TÁCH: SỐ KHUYẾT NHỊP (Không Rơi/Đảo từ T-1)')
        if sl_r == 0:
            lines.append(' 👉 KẾT QUẢ: 100% Danh mục duy trì động lượng tốt (Không có mã khuyết nhịp)')
            lines.append('')
        else:
            lines.append(f' • Danh mục {sl_r} mã: ' + ' '.join([f'{x:02d}' for x in list_rac]))
            lines.append(f' • Đạt {nhay_r} lượt.  Vốn đầu tư: {chi_r:,.0f}đ  | Doanh thu: {thu_r:,.0f}đ')
            lines.append(f' 👉 HIỆU QUẢ CỦA MÃ KHUYẾT: {lai_r_str} VNĐ ({st_r})')
            lines.append('')
        lines.append('💎 [KỊCH BẢN 3] - BÓC TÁCH: SỐ TINH HOA (Động lượng Rơi/Đảo từ T-1)')
        if sl_t == 0:
            lines.append(' 👉 KHÔNG CÓ MÃ ĐẠT CHUẨN (Toàn bộ danh mục mất động lượng)')
            lines.append('')
        else:
            lines.append(f' • Danh mục {sl_t} mã: ' + ' '.join([f'{x:02d}' for x in list_tinh_hoa]))
            lines.append(f' • Đạt {nhay_t} lượt.  Vốn đầu tư: {chi_t:,.0f}đ  | Doanh thu: {thu_t:,.0f}đ')
            lines.append(f' 👉 LỢI NHUẬN RÒNG: {lai_t_str} VNĐ ({st_t})')
            lines.append('')
        lines.append('========================================================================')
        lines.append('💡 KẾT LUẬN KIỂM TOÁN CHUYÊN SÂU:')
        if sl_r == 0:
            lines.append(' -> Đánh giá: Danh mục cấu trúc vững chắc, 100% các mã số duy trì xu hướng tích cực.')
        elif lai_r < 0:
            lines.append(f' -> Phân tích: Các mã khuyết nhịp đã làm suy giảm {-lai_r:,.0f} VNĐ lợi nhuận. Chiến lược TINH HOA là phương án bảo toàn vốn tối ưu.')
        elif lai_r > 0:
            lines.append(f' -> Lưu ý rủi ro: Nhóm số khuyết nhịp tạo ra lợi nhuận bất thường {lai_r:,.0f} VNĐ. Thị trường đang có biến động ngoài dự kiến.')
        return '\n'.join(lines)
...
def web_phan_he_5_monthly_audit(month, year, pts_per_code_base, mode):
    try:
        ...
        while curr <= end_dt:
            ...
            lines.append(f'{ngay_str:<12} | {status_str:<15} | {so_luong_lo:<7} | {von_1_phien:<14,.0f} | {nhay:<5} | {thuong:<14,.0f} | {lai:>+15,.0f} | {luy_ke_thang:>+12,.0f}')
            curr += timedelta(days=1)
        roi = (luy_ke_thang / cash_chi * 100) if cash_chi > 0 else 0
        lines.append('===================================================================================================================')
        lines.append(f'📝 ĐỐI SOÁT KẾ TOÁN: {total_phien_danh} PHIÊN CÓ XUẤT LỆNH')
        lines.append(f'• TỔNG DÒNG TIỀN (CASH FLOW): Giải ngân {cash_chi:,.0f} đ | Thu về {cash_thu:,.0f} đ')
        lines.append(f'• LỢI NHUẬN RÒNG & BIÊN R.O.I: {luy_ke_thang:+,.0f} VND ({roi:+.2f} %)')
        return '\n'.join(lines)
...
def web_phan_he_6_range_performance(tu_ngay_raw, den_ngay_raw, pts_per_code_base, mode):
    try:
        ...
        if not daily_records:
            lines.append('🛑 KHÔNG CÓ PHIÊN GIAO DỊCH NÀO ĐẠT ĐIỀU KIỆN XUẤT LỆNH THỰC TẾ.')
            return '\n'.join(lines)
            
        df_rec = pd.DataFrame(daily_records)
        lines.append('')
        lines.append('📊 1. BẢNG TỔNG HỢP DIỄN BIẾN THEO NĂM (YEARLY PNL BREAKDOWN)')
        lines.append('-------------------------------------------------------------------------------------------------------------------')
        lines.append(f'{"NĂM":<10} | {"PHIÊN":<7} | {"SỐ MÃ":<8} | {"VỐN ĐẦU TƯ":<14} | {"DOANH THU":<14} | {"LỢI NHUẬN RÒNG":<16} | {"ROI (%)":<8}')
        lines.append('-------------------------------------------------------------------------------------------------------------------')
        ...
        lines.append('')
        lines.append('📊 2. BẢNG TỔNG HỢP DIỄN BIẾN THEO THÁNG (MONTHLY PNL BREAKDOWN)')
        lines.append('-------------------------------------------------------------------------------------------------------------------')
        lines.append(f'{"THÁNG/NĂM":<10} | {"PHIÊN":<7} | {"WIN/LOSS":<10} | {"VỐN ĐẦU TƯ":<14} | {"DOANH THU":<14} | {"LỢI NHUẬN RÒNG":<16} | {"ROI (%)":<8}')
        lines.append('-------------------------------------------------------------------------------------------------------------------')
        ...
        lines.append('===================================================================================================================')
        lines.append(f'📝 ĐẠI KẾ TOÁN TỔNG CỘNG ({len(df_rec)} PHIÊN CÓ XUẤT LỆNH | Win: {tot_win} - Loss: {tot_loss}):')
...
with gr.Blocks(title='XSMB QUANT V36.2 PRO') as demo:
    gr.Markdown('# 🚀 XSMB QUANT V36.2 — PHIÊN BẢN THƯƠNG MẠI (TÍCH HỢP HOÀN CHỈNH)')
    gr.Markdown('*(Hệ thống Phân tích Định lượng & Quản trị Rủi ro. Đã tối ưu hóa và sửa lỗi toàn diện.)*')
    with gr.Row():
    ...
    with gr.Column(visible=False) as col_2:
        ...
        with gr.Row():
            ...
            mode_2 = gr.Radio(choices=MODES, value='🚀 Giao Dịch T-7 ĐỘNG LƯỢNG TỐI ƯU (Cải Tiến Quant V36.2)', label='Chiến lược Áp dụng')
        btn_2 = gr.Button('🔍 XUẤT KHUYẾN NGHỊ GIAO DỊCH', variant='primary')
        out_2 = gr.Textbox(label='Hồ sơ Giao dịch V36.2', lines=16)
    ...
    with gr.Column(visible=False) as col_5:
        with gr.Row():
            ...
            mode_5 = gr.Radio(choices=MODES, value='🚀 Giao Dịch T-7 ĐỘNG LƯỢNG TỐI ƯU (Cải Tiến Quant V36.2)', label='Chiến lược Áp dụng')
    ...
    with gr.Column(visible=False) as col_6:
        with gr.Row():
            ...
            mode_6 = gr.Radio(choices=MODES, value='🚀 Giao Dịch T-7 ĐỘNG LƯỢNG TỐI ƯU (Cải Tiến Quant V36.2)', label='Chiến lược Áp dụng')
