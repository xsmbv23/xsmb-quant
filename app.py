...
# ==============================================================================
# 🧬 HẠ TẦNG QUANT V36.1.1 PRO - PHIÊN BẢN ĐỘNG LƯỢNG TỐI ƯU (COMMERCIAL EDITION)
# ==============================================================================
...
# ==============================================================================
# 🎯 LÕI THUẬT TOÁN: BỘ LỌC ĐA CHẾ ĐỘ (ĐÃ SỬA VÀ TÍCH HỢP CHUẨN V36.1.1)
# ==============================================================================
...
def web_phan_he_6_range_performance(tu_ngay_raw, den_ngay_raw, pts_per_code_base, mode):
    try:
        ...
        report += f"===================================================================================================================\\n"
        report += f"📈 KẾT QUẢ TỪ {start_dt.strftime('%d/%m/%Y')} ĐẾN {end_dt.strftime('%d/%m/%Y')} (CHIẾN LƯỢC: {mode})\\n"
        report += f"===================================================================================================================\\n\\n"
        ...
        if not daily_records:
            return report + "🛑 KHÔNG CÓ PHIÊN GIAO DỊCH NÀO ĐẠT ĐIỀU KIỆN XUẤT LỆNH THỰC TẾ."

        df_rec = pd.DataFrame(daily_records)

        # 1. BẢNG TỔNG HỢP THEO NĂM (YEARLY BREAKDOWN)
        report += f"📊 1. BẢNG TỔNG HỢP DIỄN BIẾN THEO NĂM (YEARLY PNL BREAKDOWN)\\n"
        report += f"-------------------------------------------------------------------------------------------------------------------\\n"
        report += f"{'NĂM':<10} | {'PHIÊN':<7} | {'SỐ MÃ':<8} | {'VỐN ĐẦU TƯ':<14} | {'DOANH THU':<14} | {'LỢI NHUẬN RÒNG':<16} | {'ROI (%)':<8}\\n"
        report += f"-------------------------------------------------------------------------------------------------------------------\\n"
        ...
        for year, g_y in df_rec.groupby('year'):
            ...
            report += f"Năm {year:<6} | {len(g_y):<7} | {g_y['codes'].sum():<8} | {p_chi:<14,.0f} | {p_thu:<14,.0f} | {p_lai:>+16,.0f} | {p_roi:>+7.2f}%\\n"

        # 2. BẢNG TỔNG HỢP THEO THÁNG (MONTHLY BREAKDOWN)
        report += f"\\n📊 2. BẢNG TỔNG HỢP DIỄN BIẾN THEO THÁNG (MONTHLY PNL BREAKDOWN)\\n"
        report += f"-------------------------------------------------------------------------------------------------------------------\\n"
        report += f"{'THÁNG/NĂM':<10} | {'PHIÊN':<7} | {'WIN/LOSS':<10} | {'VỐN ĐẦU TƯ':<14} | {'DOANH THU':<14} | {'LỢI NHUẬN RÒNG':<16} | {'ROI (%)':<8}\\n"
        report += f"-------------------------------------------------------------------------------------------------------------------\\n"
        ...
        for m_str, g_m in df_rec.groupby('month_str', sort=False):
            ...
            report += f"Tháng {m_str:<5} | {len(g_m):<7} | {wl_str:<10} | {m_chi:<14,.0f} | {m_thu:<14,.0f} | {m_lai:>+16,.0f} | {m_roi:>+7.2f}%\\n"

        # 3. TỔNG KẾT TOÀN CHU KỲ
        ...
        report += f"===================================================================================================================\\n"
        report += f"📝 ĐẠI KẾ TOÁN TỔNG CỘNG ({len(df_rec)} PHIÊN CÓ XUẤT LỆNH | Win: {tot_win} - Loss: {tot_loss}):\\n"
...
# ==============================================================================
# 🎨 GIAO DIỆN V36.1.0 (BẢNG ĐIỀU KHIỂN GRADIO - BẢN TÍCH HỢP HOÀN CHỈNH)
# ==============================================================================
...
with gr.Blocks(title="XSMB QUANT V36.1.0 PRO") as demo:
    gr.Markdown("# 🚀 XSMB QUANT V36.1.0 — PHIÊN BẢN THƯƠNG MẠI (TÍCH HỢP HOÀN CHỈNH)")
    ...
    with gr.Column(visible=False) as col_2:
        ...
        with gr.Row():
            ...
            mode_2 = gr.Radio(choices=MODES, value="🚀 Giao Dịch T-7 ĐỘNG LƯỢNG TỐI ƯU (Cải Tiến Quant V36.1.1)", label="Chiến lược Áp dụng")
    ...
    with gr.Column(visible=False) as col_5:
        with gr.Row():
            ...
            mode_5 = gr.Radio(choices=MODES, value="🚀 Giao Dịch T-7 ĐỘNG LƯỢNG TỐI ƯU (Cải Tiến Quant V36.1.1)", label="Chiến lược Áp dụng")
    ...
    with gr.Column(visible=False) as col_6:
        with gr.Row():
            ...
            mode_6 = gr.Radio(choices=MODES, value="🚀 Giao Dịch T-7 ĐỘNG LƯỢNG TỐI ƯU (Cải Tiến Quant V36.1.1)", label="Chiến lược Áp dụng")
