...
# ==============================================================================
# 🎯 LÕI THUẬT TOÁN HỢP NHẤT: BỘ LỌC ĐA CHẾ ĐỘ
# ==============================================================================
...
def web_phan_he_2_predict(pts_per_code_base, mode):
    try:
        ...
        if dan is None: 
            return f"🎯 LỆNH KỲ: {next_predict_dt.strftime('%d/%m/%Y')}\n=======================================================\n🛑 CẢNH BÁO: Dữ liệu tham chiếu {msg} bị khuyết. HỆ THỐNG KHÓA LỆNH ĐỂ BẢO TOÀN VỐN."
...
def web_phan_he_3_risk_audit(base_pts, sim_size):
    try:
        ...
        von_ngay = so_luong_lo * base_pts * COST_PER_POINT
        res = f"📊 BẢNG MÔ PHỎNG LÃI LỖ GIẢ ĐỊNH (KHI DÀN CÓ {so_luong_lo} SỐ)\n"
        res += f"TỔNG CHI PHÍ 1 NGÀY: {von_ngay:,.0f} VNĐ\n"
        res += f"====================================================================\n"
...
def web_phan_he_4_single_day_backtest(ngay_raw, pts_per_code_base):
    try:
        ...
        if ngay_str_t7 not in db:
            return f"📡 KIỂM TOÁN NGÀY {ngay_str}\n=======================================================\n🔭 LỖI: Ngày T-7 ({ngay_str_t7}) bị khuyết dữ liệu. Không thể phân lập chu kỳ!"
        ...
        report = f"📡 BÁO CÁO ĐỐI SOÁT NGÀY: {ngay_str} (MỨC CƯỢC: {base_pts}đ/con)\n"
        report += f"========================================================================\n\n"
...
def web_phan_he_5_monthly_audit(month, year, pts_per_code_base, mode):
    try:
        ...
        if start_dt > end_dt: return f"🛑 BÁO CÁO: Tháng {thang:02d}/{nam} chưa có dữ liệu trong Database."
        
        report = f"📊 AUDIT TÀI CHÍNH THÁNG {thang:02d}/{nam} (CHẾ ĐỘ: {mode}):\n"
        report += f"===================================================================================================================\n"
        ...
        report += f"===================================================================================================================\n"
        report += f"📝 ĐỐI SOÁT KẾ TOÁN: {total_phien_danh} NGÀY CÓ LỆNH\n"
...
def web_phan_he_6_range_performance(tu_ngay_raw, den_ngay_raw, pts_per_code_base, mode):
    try:
        ...
        while curr <= end_dt:
            ...
            dan, msg = get_signal_v34(curr, db, mode)
            if dan is None:
                rep += f"{ngay_str:<10} | {'🔭 QUAN SÁT':<15} | {'0':<6} | {'-':<16} | {'-':<5} | {msg:<15} | {total_lai:>+12,.0f}\n"
                curr += timedelta(days=1)
                continue
...
with gr.Blocks(title="XSMB QUANT V34.4") as demo:
    ...
    with gr.Tab("🔍 [4] Tách Lớp 1 Ngày"):
        ...
        btn_4 = gr.Button("📡 SO SÁNH HIỆU QUẢ DÒNG TIỀN", variant="primary")
        out_4 = gr.Textbox(label="Báo cáo Tách Lớp Dòng Tiền", lines=22)
