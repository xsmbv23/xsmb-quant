...
# ==============================================================================
# 🧬 HẠ TẦNG QUANT V30.3 - KHUNG 3 NGÀY (DEEP-TESTED MASTER)
# ==============================================================================
VERSION = "V30.3 KHUNG 3 NGÀY (STRICT INTEGRITY)"
DATA_FILE = "Ket_Qua_Loto27.xlsx"
COST_PER_POINT = 21700
WIN_PER_NHAY = 80000

# KỶ LUẬT VỐN: GẤP THẾP ĐÚNG 3 NGÀY, KHÔNG ĐƯỢC PHÉP VƯỢT QUÁ.
...
def doc_database_tu_excel():
    ...
    try:
        ...
        return db, f"🟢 NẠP THÀNH CÔNG {len(db)} NGÀY DỮ LIỆU. TOÀN VẸN DỮ LIỆU ĐƯỢC BẢO ĐẢM."
...
# ==============================================================================
# 🎯 LÕI THUẬT TOÁN: QUÉT KHUNG & ÉP KỶ LUẬT 3 NGÀY
# ==============================================================================
def tim_cap_nuoi(target_dt, db):
    hist_days = []
    curr_t = target_dt - timedelta(days=1)
    
    # Ép buộc phải có 30 ngày lịch sử để phân tích nhịp. Thiếu -> Trả về Rỗng.
    for _ in range(30):
    ...
    # Nếu ngày T-1 không có dữ liệu, KHÔNG CẤP LỆNH.
    ...
    for i in range(10, 100):
        ...
        # Tìm Cặp Nén: Vắng bóng 4-6 ngày. Tần suất 30 ngày >= 6 lần.
        if 4 <= days_missed <= 6:
    ...
    # Chọn cặp có nhịp về dày nhất trong 30 ngày qua
    sorted_pairs = sorted(pair_scores.items(), key=lambda k: k[1], reverse=True)
    return sorted_pairs[0][0]

def truy_xuat_trang_thai_khung(db, target_dt):
    """
    DEEP-TESTED: Quét lại toàn bộ lịch sử từ mốc Date = 0 của DB đến Target_DT.
    Đảm bảo tính đồng bộ tuyệt đối về chu kỳ Khung ở tất cả các Menu.
    """
    ...
    while curr < target_dt:
        ...
        if khung_active and ngay_str in db:
            ...
            if nhay > 0:
                khung_active = False # WIN -> Giải phóng khung
            else:
                # KỶ LUẬT THÉP: NGÀY THỨ 3 KHÔNG NỔ -> CẮT LỖ LẬP TỨC.
                if day_in_khung == 3: khung_active = False 
                else: day_in_khung += 1
...
# ==============================================================================
# 🖥️ FULL 7 PHÂN HỆ GIAO DIỆN (BÁO CÁO TRUNG THỰC - KHÔNG TÔ HỒNG)
# ==============================================================================

def web_phan_he_1_sync():
    db, msg = doc_database_tu_excel()
    latest_dt, next_predict_dt = lay_ngay_chot_tu_excel(db)
    res = f"📡 HỆ THỐNG V30.3 (KHUNG 3 NGÀY) - CHẾ ĐỘ STRICT AUDIT:\n"
    res += f"---------------------------------------------------------------------------------\n"
...
def web_phan_he_2_predict(pts_per_code_base):
    try:
        ...
        if not active:
            ...
            if not pair:
                return f"🎯 LỆNH KỲ: {next_predict_dt.strftime('%d/%m/%Y')}\n=======================================================\n🛑 CẢNH BÁO: Dữ liệu chưa đủ độ chín hoặc không có Cặp nén. HỆ THỐNG CẤM GIAO DỊCH."
            day_idx = 1
        ...
        res = f"🎯 XUẤT LỆNH V30.3 CHO KỲ: {next_predict_dt.strftime('%d/%m/%Y')}\n"
        res += f"=======================================================\n"
...
def web_phan_he_4_single_day_backtest(ngay_raw, pts_per_code_base):
    try:
        ...
        if not active:
            ...
            if not pair: return f"📡 KIỂM TOÁN NGÀY {ngay_str}\n=======================================================\n🔭 LỊCH SỬ CHƯA ĐỦ ĐỘ CHÍN ĐỂ CẤP LỆNH. GHI NHẬN ĐỨNG NGOÀI."
            day_idx = 1
...
def web_phan_he_5_monthly_audit(month, year, pts_per_code_base):
    try:
        ...
        report = f"📊 AUDIT TÀI CHÍNH THÁNG {thang:02d}/{nam} (SỐ LIỆU THÔ - KHÔNG TÔ HỒNG):\n"
        report += f"=================================================================================================================\n"
        report += f"{'NGÀY':<10} | {'TRẠNG THÁI':<18} | {'CẶP VỊ THẾ':<10} | {'NGÀY VÀO':<8} | {'CHI PHÍ':<12} | {'KQ':<5} | {'BIẾN ĐỘNG RÒNG':<15}\n"
        report += f"=================================================================================================================\n"
        ...
        while curr <= end_dt:
            ...
            if not khung_active:
                ...
                if cap:
                    ...
                    report += f"{ngay_str:<10} | {'🔭 QUAN SÁT/NO DATA':<18} | {'-':<10} | {'-':<8} | {0:<12} | {'-':<5} | {0:<15}\n"
                    curr += timedelta(days=1)
                    continue
            ...
            if ngay_str in db:
                ...
                if nhay > 0:
                    ...
                    report += f"{ngay_str:<10} | {'🟢 WIN (CHỐT LÃI)':<18} | {p_str:<10} | {day_in_khung:<8} | {von_ngay:<12,.0f} | {nhay:<5} | {lai_khung:>+15,.0f}\n"
                    khung_active = False
                else:
                    if day_in_khung == 3:
                        lai_khung = -von_khung_hien_tai
                        luy_ke_thang += lai_khung
                        report += f"{ngay_str:<10} | {'🔴 LOSS (CẮT LỖ)':<18} | {p_str:<10} | {day_in_khung:<8} | {von_ngay:<12,.0f} | {0:<5} | {lai_khung:>+15,.0f}\n"
                        khung_active = False
                    else:
                        report += f"{ngay_str:<10} | {'⏳ PENDING (GỒNG)':<18} | {p_str:<10} | {day_in_khung:<8} | {von_ngay:<12,.0f} | {0:<5} | {'...':<15}\n"
                        day_in_khung += 1
            curr += timedelta(days=1)
            
        report += f"=================================================================================================================\n"
        report += f"💰 TỔNG LỢI NHUẬN RÒNG GHI NHẬN TRONG THÁNG: {luy_ke_thang:+,.0f} VND\n"
        return report
...
def web_phan_he_6_range_performance(tu_ngay_raw, den_ngay_raw, pts_per_code_base):
    try:
        ...
        rep += "="*115 + "\n"
        rep += f"{'NGÀY':<10} | {'TRẠNG THÁI':<18} | {'VỊ THẾ':<8} | {'NGÀY VÀO':<8} | {'CHI PHÍ':<12} | {'KQ':<5} | {'BIẾN ĐỘNG':<12} | {'LŨY KẾ':<12}\n"
        rep += "="*115 + "\n"
        ...
        while curr <= end_dt:
            ...
            if not khung_active:
                ...
                if cap:
                    ...
                    rep += f"{ngay_str:<10} | {'🔭 NO DATA':<18} | {'-':<8} | {'-':<8} | {0:<12} | {'-':<5} | {0:<12} | {total_lai:>+12,.0f}\n"
                    curr += timedelta(days=1)
                    continue
            ...
            if ngay_str in db:
                ...
                if nhay > 0:
                    ...
                    total_lai += lai_khung
                    k_thang += 1
                    rep += f"{ngay_str:<10} | {'🟢 WIN (CHỐT LÃI)':<18} | {p_str:<8} | {day_in_khung:<8} | {von_ngay:<12,.0f} | {nhay:<5} | {lai_khung:>+12,.0f} | {total_lai:>+12,.0f}\n"
                    khung_active = False 
                else:
                    if day_in_khung == 3:
                        lai_khung = -von_khung_hien_tai
                        total_lai += lai_khung
                        k_thua += 1
                        rep += f"{ngay_str:<10} | {'🔴 LOSS (CẮT LỖ)':<18} | {p_str:<8} | {day_in_khung:<8} | {von_ngay:<12,.0f} | {0:<5} | {lai_khung:>+12,.0f} | {total_lai:>+12,.0f}\n"
                        khung_active = False 
                    else:
                        rep += f"{ngay_str:<10} | {'⏳ PENDING (GỒNG)':<18} | {p_str:<8} | {day_in_khung:<8} | {von_ngay:<12,.0f} | {0:<5} | {'...':<12} | {total_lai:>+12,.0f}\n"
                        day_in_khung += 1
            curr += timedelta(days=1)
...
# ==============================================================================
# 🎨 GIAO DIỆN GRADIO V30.3 (KHÓA GIAO DIỆN CHẶT CHẼ)
# ==============================================================================
db_init, _ = doc_database_tu_excel()
latest_dt_init, next_predict_dt_init = lay_ngay_chot_tu_excel(db_init)

with gr.Blocks(title="XSMB QUANT V30.3") as demo:
    gr.Markdown("# 🚀 XSMB QUANT V30.3 — KHUNG 3 NGÀY (STRICT AUDIT MASTER)")
    ...
    with gr.Tab("🎯 [2] Lệnh Chốt Kế Tiếp"):
        ...
        out_2 = gr.Textbox(label="Hồ sơ Lệnh V30.3", lines=12)
    ...
    with gr.Tab("📊 [5] Kiểm Toán Theo Tháng"):
        with gr.Row():
            # Slider bắt buộc từ 1 -> 12. Không cho phép gõ ký tự lạ.
        ...
        out_5 = gr.Textbox(label="Nhật ký Audit", lines=18)
    ...
    with gr.Tab("📈 [6] Kiểm Toán Tổng Chu Kỳ"):
        ...
        out_6 = gr.Textbox(label="Báo cáo Tổng", lines=18)
