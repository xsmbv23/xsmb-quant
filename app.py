...
# ==============================================================================
class Config:
    VERSION = "V36.6 PRO ALGO (BẢN KIỂM TOÁN LÕI - CHỐNG DỮ LIỆU RÁC 100%)"
...
class Utils:
    ...
    def chuan_hoa_ngay(ngay_raw):
        ...
        try:
            ...
            # Tuyệt đối chặn dữ liệu tương lai (Lookahead bias / Lỗi Crawler rác)
...
class Crawler:
    ...
    def fetch_source_1():
        ...
        response = requests.get("[https://xoso.com.vn/xsmb.html](https://xoso.com.vn/xsmb.html)", headers=headers, timeout=10)
        ...
        # Chỉ lấy text chứa ngày chuẩn 4 số năm (tránh cào dính số đt rác)
    ...
    def fetch_source_2():
        headers = {'User-Agent': 'Mozilla/5.0'}
        response = requests.get("[https://kqxs.vn/mien-bac](https://kqxs.vn/mien-bac)", headers=headers, timeout=10)
        if response.status_code != 200: return False, None, None
        soup = BeautifulSoup(response.content, 'html.parser')
...
class DatabaseManager:
    ...
    def update_excel_from_crawler():
        ...
        if not res_date: return f"🛑 CẬP NHẬT TỰ ĐỘNG THẤT BẠI: Ngày Crawler trả về '{date_str_crawled}' không hợp lệ hoặc nằm ở Tương lai!"
...
def create_ui():
    ...
    with gr.Blocks(title="XSMB QUANT V36.6 PRO") as demo:
        gr.Markdown("# 🚀 XSMB QUANT V36.6 PRO — BẢN KIỂM TOÁN LÕI (CHỐNG DỮ LIỆU RÁC 100%)")
        gr.Markdown("*(Đã gỡ bỏ toàn bộ tính năng dư thừa gây nghẽn Server Render. Tối ưu cực hạn Crawler bắt ngày giờ siêu chuẩn và tự động loại bỏ rủi ro lấy dữ liệu tương lai.)*")
