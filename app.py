...
# ==============================================================================
# 🧬 HẠ TẦNG QUANT V36.1.0 PRO - PHIÊN BẢN TÍCH HỢP HOÀN CHỈNH (COMMERCIAL EDITION)
# ==============================================================================
...
def web_phan_he_6_range_performance(
  ...
  try:
    ...
    report += f"===================================================================================================================\n"
    report += (
        f"📈 KẾT QUẢ TỪ GIAI ĐOẠN {start_dt.strftime('%d/%m/%Y')} ĐẾN"
        f" {end_dt.strftime('%d/%m/%Y')} (CHIẾN LƯỢC: {mode})\n"
    )
    report += f"-------------------------------------------------------------------------------------------------------------------\n"
    report += (
        f"{'NGÀY G.DỊCH':<12} | {'TRẠNG THÁI':<15} | {'SỐ MÃ':<7} |"
        f" {'VỐN ĐẦU TƯ':<14} | {'LƯỢT':<5} | {'LỢI NHUẬN PHIÊN':<16} |"
        f" {'LŨY KẾ':<12}\n"
    )
    report += f"-------------------------------------------------------------------------------------------------------------------\n"
    ...
    while curr <= end_dt:
      ...
      if len(dan) == 0:
        report += (
            f"{ngay_str:<12} | {'🔭 THEO DÕI':<15} | {'0':<7} | {'-':<14} |"
            f" {'-':<5} | {'[KHÔNG TÍN HIỆU]':<16} | {total_lai:>+12,.0f}\n"
        )
        curr += timedelta(days=1)
        continue
      ...
      report += (
          f"{ngay_str:<12} | {status_str:<15} | {so_luong_lo:<7} |"
          f" {von_1_phien:<14,.0f} | {nhay:<5} | {lai:>+16,.0f} |"
          f" {total_lai:>+12,.0f}\n"
      )
      curr += timedelta(days=1)
    ...
    report += f"===================================================================================================================\n"
    report += (
        f"📊 HIỆU SUẤT GIAO DỊCH: {trades} Phiên Thực Tế (Win: {k_thang} | Loss:"
        f" {k_thua})\n"
    )
    report += (
        f"📝 TỔNG DÒNG TIỀN    : Giải ngân {cash_chi:,.0f} đ | Thu về"
        f" {cash_thu:,.0f} đ\n"
    )
    report += (
        f"💰 LỢI NHUẬN RÒNG    : {total_lai:+,.0f} VNĐ (ROI: {roi:+.2f} %)\n"
    )
    return report
  except Exception as e:
